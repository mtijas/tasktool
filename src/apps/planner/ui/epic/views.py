...
def list_epics(request):
    ...
    return render(request, 'planner/epic/epic_table.html', {'epics': epics})

@login_required
@permission_required('epic.view_epic')
def get_table(request):
    epics = Epic.objects.order_by('title')
    return render(request, 'planner/epic/table_rows.html', {'epics': epics})
...
class EpicCreateView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, theme_id=None):
        ...
        return render(request, 'planner/epic/quick_edit_form.html', {'form': form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            epic.save()
            for theme_id in form.cleaned_data['themes']:
                epic.theme_set.add(theme_id)
            ...
            response = render(request, 'planner/epic/quick_edit_form.html', {'form': form})
        return response
...
class EpicUpdateView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, epic_id):
        ...
        themes = [(t.id, t.title) for t in Theme.objects.all()]
        initial_themes = [t.id for t in epic.theme_set.all()]
        form = EpicEditForm(initial={'title': epic.title, 'description': epic.description, 'themes': initial_themes}, themes=themes)
        return render(request, 'planner/epic/quick_edit_form.html', {'form': form, 'epic': epic})
